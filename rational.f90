...
subroutine print_values_wp(arr, name, last)
   ...
   ! open (unit = 90, file = "vals.json", status="old")
   write(90, *) field
   do i = 1, length
       if (i == length) then
           write(90, *) arr(i)
...
subroutine print_values(arr, name)
   real, intent(in) :: arr(:)
   Character(*) :: name
   integer :: i, length = 0
   Character(len=12) :: field
...
subroutine get_dispersion_energy(self, mol, trans, cutoff, r4r2, c6, energy)
   ...
   !> Dispersion energy
   real(wp), intent(inout) :: energy(:)
   integer :: iat, jat, izp, jzp, jtr, cnt
   real(wp) :: vec(3), r2, cutoff2, r0ij, rrij, c6ij, t6, t8, edisp, dE
   integer :: total
   real, DIMENSION(:), ALLOCATABLE :: rrijs, r2s
   real(wp), DIMENSION(:), ALLOCATABLE :: t6s, t8s, edisps, des



   cutoff2 = cutoff*cutoff
   total = (mol%nat * (mol%nat - 1) / 2)
   print *, 'total', total
   ALLOCATE (rrijs(total))
   ALLOCATE (r2s(total))
   ALLOCATE (t6s(total))
   ALLOCATE (t8s(total))
   ALLOCATE (edisps(total))
   ALLOCATE (des(total))
   ...
   ! print *, "trans size", size(trans, 2) == 1 for the system I looked at
   cnt = 0
   do iat = 1, mol%nat
      izp = mol%id(iat)
      do jat = 1, iat
         ...
         do jtr = 1, size(trans, 2)
            ...
            ! print *, "carts1 = ", mol%xyz(:, iat)
            ! print *, "carts2 = ", mol%xyz(:, jat)
            ! print *, vec
            ...
            ! print *,mol%xyz(1, iat), iat, jat, r2, edisp, dE
            ! print *, iat, jat, dE, c6ij
   ...
   open (unit = 90, file = "vals.json", status="new")
   write(90, *) "{"
   call print_values(rrijs, 'rrijs')
   call print_values(r2s, 'r2s')
   call print_values_wp(t6s, 't6s', 0)
   call print_values_wp(t8s, 't8s', 0)
   call print_values_wp(edisps, 'edisps', 0)
   call print_values_wp(energy, 'energy', 0)
   call print_values_wp(des, 'des', 1)
   write(90, *) "}"
   close(90)
